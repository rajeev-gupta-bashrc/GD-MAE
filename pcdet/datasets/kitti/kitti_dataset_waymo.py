...
from ...utils import box_utils, calibration_waymo, common_utils, object3d_kitti
from ..dataset import DatasetTemplate


class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, segment_name=None, logger=None):
        """
        Args:
            root_path:
            dataset_cfg:
        ...
        self.segment_name = 'segment-13182548552824592684_4160_250_4180_250_with_camera_label'
        self.split = self.dataset_cfg.DATA_SPLIT[self.mode]
        # self.root_split_path = self.root_path / ('training' if self.split != 'test' else 'testing')
        self.root_split_path = self.root_path / self.segment_name
    ...
    def map_to_021(self, points):
        return (points - points.min()) / (points.max() - points.min())
    
    def get_intensity_from_xyz(self, points):
        intensity = np.sqrt(np.sum(points**2, axis=1))
        intensity =             1 - self.map_to_021(intensity)
        height_compensation =   self.map_to_021(points[:, 2])
        intensity = intensity * height_compensation
        return intensity
        
    def get_lidar(self, idx):
        # lidar_file = self.root_split_path / 'velodyne_reduced' / ('%s.bin' % idx)
        lidar_file = self.root_split_path / 'velodyne' / ('%s.bin' % idx)
        # points = self.client.load_to_numpy(str(lidar_file), dtype=np.float32).reshape(-1, 3)      #x, y, z
        ## intensity
        # points = np.concatenate((points, np.ones((points.shape[0], 1))), axis=1)                  #model needs intensity
        # intensity = self.get_intensity_from_xyz(points)
        # points = np.column_stack((points, intensity))
        
        # newly created bin files x, y, z, intensity
        points = self.client.load_to_numpy(str(lidar_file), dtype=np.float32).reshape(-1, 4)    
        return points   #x, y, z, intensity

    def get_image(self, idx):
        img_file = self.root_split_path / 'image' / 'FRONT' / ('%s.png' % idx)
        assert self.client.exists(img_file)
        image = self.client.load_img(str(img_file)).astype(np.float32)  # BGR, uint8
        # crop to kitti shape
    ...
    def get_calib(self, idx):
        ...
        return calibration_waymo.Calibration(calib_file, self.client)
